...
void ExitHalf(int direction) {
   ...
   Comment(StringConcatenate("Exiting half trades on ", Symbol()));
   ...
   for (int i = OrdersTotal()-1; i >= 0; i--) {           
      ...
      if(OrderSymbol() == Symbol()) {
         total++;
         if (OrderType() == OP_BUY && ( direction == LONG || direction == ALL) ) { Exit(OrderTicket(), LONG, OrderLots()/2.0, CLR_NONE); }
         if (OrderType() == OP_SELL && ( direction == SHORT || direction == ALL) ) { Exit(OrderTicket(), SHORT, OrderLots()/2.0, CLR_NONE); }
      }
   }
